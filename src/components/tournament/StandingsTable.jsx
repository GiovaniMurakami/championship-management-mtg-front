...
function CollapseToggle({ collapsed, onToggle, label = "standings" }) {
  return (
    <button
      ...
      className="inline-flex h-9 items-center gap-1.5 px-3 border border-[rgba(217,180,255,0.25)] rounded-xl bg-white/[0.04] text-[#d7cce9] text-[0.75rem] font-semibold cursor-pointer transition-[color,border-color,background-color] duration-150 hover:text-white hover:border-[rgba(199,149,255,0.45)] hover:bg-white/[0.08] shrink-0 active:scale-[0.98]"
    >
      <svg
        width="12"
        height="12"
...
export function StandingsTable({
  ...
  if (!standings || standings.length === 0) {
    return (
      <section className="border border-line rounded-2xl p-5 bg-[linear-gradient(160deg,rgba(34,19,69,0.6),rgba(15,10,29,0.85))] shadow-[0_4px_20px_rgba(3,2,8,0.3)] animate-[slide-up_400ms_ease-out]">
        <h2 className="m-0 mb-4 font-['Bebas_Neue',sans-serif] text-[1.5rem] tracking-[0.04em] text-text-main">Standings</h2>
        <p className="text-text-soft text-[0.9rem] m-0">Nenhum dado de standings disponível.</p>
      </section>
    );
  }
  ...
  if (compact) {
    return (
      <section className="border border-line rounded-2xl bg-[linear-gradient(160deg,rgba(34,19,69,0.6),rgba(15,10,29,0.85))] shadow-[0_4px_20px_rgba(3,2,8,0.3)] animate-[slide-up_400ms_ease-out] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between gap-2 px-4 pt-4 pb-3 flex-shrink-0">
          <h2 className="m-0 font-['Bebas_Neue',sans-serif] text-[1.4rem] tracking-[0.04em] text-text-main">{sectionTitle}</h2>
          <div className="flex items-center gap-2 flex-shrink-0">
            <span className="text-[0.72rem] font-semibold text-text-soft bg-[rgba(167,79,255,0.12)] border border-line rounded-full px-[0.55rem] py-[0.15rem]">
        ...
        <div className="relative px-4 pb-3 flex-shrink-0">
          <input
            className="w-full pl-3 pr-[1.85rem] py-[0.3rem] border border-[rgba(199,149,255,0.25)] rounded-full bg-[rgba(167,79,255,0.07)] text-text-main text-[0.78rem] font-['inherit'] outline-none transition-[border-color] duration-[250ms] placeholder:text-text-soft focus:border-[rgba(199,149,255,0.5)]"
            type="text"
          ...
          />
          {search ? (
            <button type="button" className="absolute right-[1.15rem] top-1/2 -translate-y-1/2 bg-transparent border-none text-text-soft text-[1rem] leading-none cursor-pointer hover:text-text-main" onClick={() => setSearch("")} aria-label="Limpar busca">x</button>
          ) : (
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" aria-hidden="true" className="absolute right-[1.35rem] top-1/2 -translate-y-1/2 text-text-soft pointer-events-none">
        ...
        <div className="grid grid-cols-[2rem_1fr_auto] gap-x-2 px-4 pb-[0.35rem] flex-shrink-0 border-b border-[rgba(255,255,255,0.06)]">
          <span className="text-[0.65rem] font-bold uppercase tracking-[0.08em] text-brand text-center">#</span>
          <span className="text-[0.65rem] font-bold uppercase tracking-[0.08em] text-brand">Jogador</span>
          <span className="text-[0.65rem] font-bold uppercase tracking-[0.08em] text-brand text-right">Pts</span>
        </div>

        <div className="pb-2">
          {filtered.length === 0 ? (
            <p className="text-text-soft text-[0.82rem] m-0 px-4 pt-3">Nenhum jogador encontrado.</p>
          ) : (
                  ...
                  <div className={`grid grid-cols-[2rem_1fr_auto] gap-x-2 items-center px-4 py-[0.4rem] border-b border-[rgba(255,255,255,0.04)] transition-colors duration-150 hover:bg-[rgba(167,79,255,0.05)] ${rowAccent}`}>
                    <span className="text-center flex-shrink-0">
                      {isTop3 ? (
                        ...
                        <span className="text-[0.75rem] text-text-soft font-semibold">{posicao}</span>
                      )}
                    </span>
                    ...
                    <span className={`text-[0.82rem] font-bold flex-shrink-0 ${posicao <= 8 && !player.dropped ? "text-[#fde68a]" : "text-text-soft"}`}>
  ...
  return (
    <section id="standings-panel" className="border border-line rounded-2xl p-5 bg-[linear-gradient(160deg,rgba(34,19,69,0.6),rgba(15,10,29,0.85))] shadow-[0_4px_20px_rgba(3,2,8,0.3)] animate-[slide-up_400ms_ease-out] max-md:p-4 max-w-full min-w-0">
      <div className={`flex items-center justify-between gap-3 flex-wrap max-md:flex-col max-md:items-stretch ${collapsed ? "mb-0" : "mb-4"}`}>
        <h2 className="m-0 font-['Bebas_Neue',sans-serif] text-[1.5rem] tracking-[0.04em] text-text-main max-md:text-[1.35rem]">{sectionTitle}</h2>
        <div className="flex items-center gap-[0.6rem] flex-wrap max-md:w-full">
          <span className="inline-flex h-9 items-center text-[0.72rem] font-semibold text-[#c9bddc] bg-[rgba(167,79,255,0.08)] border border-line rounded-full px-3 flex-shrink-0">
            {totalInscritos ?? standings.length} inscritos
          </span>
          <CollapseToggle
            collapsed={collapsed}
          ...
          {!collapsed && standings.length > 5 && (
            <div className="relative flex h-9 items-center max-md:flex-1">
              <input
                className="h-9 pl-3 pr-[1.8rem] border border-[rgba(199,149,255,0.3)] rounded-xl bg-[rgba(255,255,255,0.035)] text-text-main text-[0.78rem] font-['inherit'] outline-none w-44 max-md:w-full transition-[border-color,background,width] duration-[250ms] placeholder:text-[#9f92b5] focus:border-[rgba(199,149,255,0.55)] focus:bg-[rgba(167,79,255,0.09)] focus:w-56 max-md:focus:w-full"
                type="text"
                placeholder="Buscar jogador..."
                value={search}
                onChange={(e) => setSearch(e.target.value)}
              ...
              {search ? (
                <button
                  type="button"
                  className="absolute right-[0.4rem] bg-transparent border-none text-text-soft text-[1rem] leading-none cursor-pointer px-[0.1rem] flex items-center hover:text-text-main"
                  onClick={() => setSearch("")}
                ...
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" aria-hidden="true" className="absolute right-[0.55rem] text-text-soft pointer-events-none flex-shrink-0">
      ...
      {!collapsed && filtered.length === 0 && (
        <p className="text-text-soft text-[0.9rem] m-0">Nenhum jogador encontrado para "{search}".</p>
      )}

      {!collapsed && filtered.length > 0 && (
        <>
          <div className="rounded-xl border border-line overflow-x-auto max-md:hidden">
            <table className="w-full table-fixed border-collapse text-[0.88rem]">
              <thead className="bg-[#21133a] sticky top-0 z-10 shadow-[0_1px_0_rgba(217,180,255,0.15)]">
                <tr>
                  <th className="w-[3%] text-center px-2 py-[0.65rem] text-[0.75rem] font-bold uppercase tracking-[0.06em] text-brand">#</th>
                  <th className="w-[22%] px-2 py-[0.65rem] text-[0.75rem] font-bold uppercase tracking-[0.06em] text-brand text-left">Jogador</th>
                  {!isRegistrationOpen && <th className="w-[5%] px-1.5 py-[0.65rem] text-[0.75rem] font-bold uppercase tracking-[0.06em] text-brand text-center">Pts</th>}
                  {!isRegistrationOpen && <th className="w-[4%] px-1.5 py-[0.65rem] text-[0.75rem] font-bold uppercase tracking-[0.06em] text-brand text-center">V</th>}
                  {!isRegistrationOpen && <th className="w-[4%] px-1.5 py-[0.65rem] text-[0.75rem] font-bold uppercase tracking-[0.06em] text-brand text-center">D</th>}
                  {!isRegistrationOpen && <th className="w-[4%] px-1.5 py-[0.65rem] text-[0.75rem] font-bold uppercase tracking-[0.06em] text-brand text-center">E</th>}
                  {!isRegistrationOpen && <th className="w-[7%] px-1.5 py-[0.65rem] text-[0.75rem] font-bold uppercase tracking-[0.06em] text-brand text-center">MWP</th>}
                  {!isRegistrationOpen && <th className="w-[7%] px-1.5 py-[0.65rem] text-[0.75rem] font-bold uppercase tracking-[0.06em] text-brand text-center">OMW%</th>}
                  {!isRegistrationOpen && <th className="w-[7%] px-1.5 py-[0.65rem] text-[0.75rem] font-bold uppercase tracking-[0.06em] text-brand text-center">GW%</th>}
                  {!isRegistrationOpen && <th className="w-[7%] px-1.5 py-[0.65rem] text-[0.75rem] font-bold uppercase tracking-[0.06em] text-brand text-center">OGW%</th>}
                  <th className={`${isFinished ? "w-[30%]" : "w-[18%]"} px-2 py-[0.65rem] text-[0.75rem] font-bold uppercase tracking-[0.06em] text-brand text-left`}>Deck</th>
                  {!isFinished && <th className="w-[12%] px-2 py-[0.65rem] text-[0.75rem] font-bold uppercase tracking-[0.06em] text-brand text-left">Check-in</th>}
                </tr>
              </thead>
              <tbody>
                {filtered.map((player, index) => {
                  ...
                  return (
                    <Fragment key={player.usuario?.id || player.usuarioId || player.id || index}>
                      <tr className={`transition-[background] duration-150 hover:bg-[rgba(167,79,255,0.06)] ${rowBorderClass}`}>
                        <td className="px-2 py-[0.55rem] border-t border-[rgba(255,255,255,0.04)] text-center font-bold text-brand">
                          {isTop3 ? (
                            ...
                            <span className="text-text-soft text-[0.82rem]">{posicao}</span>
                          )}
                        </td>
                        <td className="px-2 py-[0.55rem] border-t border-[rgba(255,255,255,0.04)] text-text-main font-semibold min-w-0">
                        ...
                        {!isRegistrationOpen && (
                          <td className="px-1.5 py-[0.55rem] border-t border-[rgba(255,255,255,0.04)] text-text-main text-center">
                        ...
                        {!isRegistrationOpen && <td className="px-1.5 py-[0.55rem] border-t border-[rgba(255,255,255,0.04)] text-text-main text-center">{player.empatesPartida ?? player.empates ?? 0}</td>}
                        {!isRegistrationOpen && <td className="px-1.5 py-[0.55rem] border-t border-[rgba(255,255,255,0.04)] text-text-soft text-[0.82rem] text-center [font-variant-numeric:tabular-nums]">{formatPct(player.mwp)}</td>}
                        {!isRegistrationOpen && <td className="px-1.5 py-[0.55rem] border-t border-[rgba(255,255,255,0.04)] text-text-soft text-[0.82rem] text-center [font-variant-numeric:tabular-nums]">{formatPct(player.omwp)}</td>}
                        {!isRegistrationOpen && <td className="px-1.5 py-[0.55rem] border-t border-[rgba(255,255,255,0.04)] text-text-soft text-[0.82rem] text-center [font-variant-numeric:tabular-nums]">{formatPct(player.gwp)}</td>}
                        {!isRegistrationOpen && <td className="px-1.5 py-[0.55rem] border-t border-[rgba(255,255,255,0.04)] text-text-soft text-[0.82rem] text-center [font-variant-numeric:tabular-nums]">{formatPct(player.ogwp)}</td>}
                        <td className="px-2 py-[0.55rem] border-t border-[rgba(255,255,255,0.04)] text-text-main relative min-w-0">
                          {isFinished ? (
                            <DeckViewButton
                              player={player}
                              token={token}
                            ...
                            <span className={getDeckStatus(player) ? "text-[#4ade80]" : "text-text-soft"}>
                              {getDeckStatus(player) ? "✓" : "-"}
                            </span>
                          )}
                        </td>
                        {!isFinished && (
                          <td className="px-2 py-[0.55rem] border-t border-[rgba(255,255,255,0.04)] text-text-main min-w-0">
                            {isCheckedIn(player) ? (
          ...
          <div className="hidden max-md:grid gap-[0.55rem] pr-1">
            {filtered.map((player, index) => {
              ...
              const mobileBorderClass = isTop3
                ...
                : isTop8 ? "border-[rgba(167,79,255,0.3)] bg-[rgba(255,255,255,0.03)]" : "border-line bg-[rgba(255,255,255,0.03)]";
                    ...
                    {isRegistrationOpen ? (
                      ...
                      <>
                        <div className="flex items-center gap-[0.35rem] mb-[0.4rem] text-[0.82rem]">
                          ...
                          <span className="text-text-soft font-normal">-</span>
                          <span className="font-semibold text-text-main">{empates}E</span>
                          {!isFinished && (
                        ...
                        <div className="flex gap-3 flex-wrap text-[0.77rem] text-text-soft [font-variant-numeric:tabular-nums]">
