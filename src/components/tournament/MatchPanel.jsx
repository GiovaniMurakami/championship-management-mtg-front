...
export function MatchPanel({ myMatch, usuario, onReportResult, onContestResult, onConfirmResult, actionLoading, torneio, isOwner, currentPlayer, onCheckin }) {
    ...
    if (!myMatch) {
        return (
            <section className="border border-line rounded-2xl p-5 bg-[linear-gradient(160deg,rgba(34,19,69,0.6),rgba(15,10,29,0.85))] shadow-[0_4px_20px_rgba(3,2,8,0.3)] animate-[slide-up_400ms_ease-out] relative overflow-hidden before:content-[''] before:absolute before:top-0 before:left-0 before:right-0 before:h-[3px] before:bg-[linear-gradient(90deg,#2ccfb4,#8e39ed,#c795ff,#8e39ed,#2ccfb4)] before:bg-[length:200%_100%] before:animate-[shimmer-bar_3s_linear_infinite] max-md:p-4">
                <h2 className="m-0 mb-4 font-['Bebas_Neue',sans-serif] text-[1.5rem] tracking-[0.04em] text-text-main">Partida Atual</h2>
                <p className="text-text-soft text-[0.9rem] m-0">Nenhuma partida na rodada atual.</p>
            </section>
        );
    }
    ...
    return (
        <section className="border border-line rounded-2xl p-5 bg-[linear-gradient(160deg,rgba(34,19,69,0.6),rgba(15,10,29,0.85))] shadow-[0_4px_20px_rgba(3,2,8,0.3)] animate-[slide-up_400ms_ease-out] relative overflow-hidden before:content-[''] before:absolute before:top-0 before:left-0 before:right-0 before:h-[3px] before:bg-[linear-gradient(90deg,#2ccfb4,#8e39ed,#c795ff,#8e39ed,#2ccfb4)] before:bg-[length:200%_100%] before:animate-[shimmer-bar_3s_linear_infinite] max-md:p-4">
            <div className="flex items-center gap-3 flex-wrap m-0 mb-5">
                <h2 className="m-0 font-['Bebas_Neue',sans-serif] text-[1.5rem] tracking-[0.04em] text-text-main">Partida Atual</h2>
                {mesaNum != null && (
            ...
            {isBye ? (
                <div className="text-center">
                    <div className="flex flex-col items-center gap-[0.35rem] p-4 px-2 border border-[rgba(142,57,237,0.4)] rounded-lg bg-[rgba(142,57,237,0.08)]">
                        <span className="text-[0.72rem] font-bold tracking-[0.1em] text-text-soft">VOCÊ</span>
                        <span className="text-[1.1rem] font-bold text-white text-center break-words">{myName}</span>
                        {myNick && (
                            <span className="text-[0.72rem] text-brand font-mono tracking-wide">{myNick}</span>
                        )}
                    </div>
                    <div className="font-['Bebas_Neue',sans-serif] text-[1.8rem] text-brand [text-shadow:0_0_12px_rgba(199,149,255,0.4)]">BYE</div>
                    <p className="text-text-soft mt-3">Você recebeu bye nesta rodada.</p>
                    {myMatch.tipoBye && (
                ...
                <>
                    <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-4 mb-5 max-[900px]:grid-cols-1 max-[900px]:gap-2 max-[900px]:text-center">
                        <div className={`flex flex-col items-center gap-[0.35rem] p-4 px-2 border rounded-lg ${leftSide.isMe ? "border-[rgba(142,57,237,0.4)] bg-[rgba(142,57,237,0.08)]" : "border-[rgba(239,68,68,0.3)] bg-[rgba(239,68,68,0.05)]"}`}>
                            <span className="text-[0.72rem] font-bold tracking-[0.1em] text-text-soft">{leftLabel}</span>
                            <span className="text-[1.1rem] font-bold text-white text-center break-words">{leftSide.name}</span>
                            {leftSide.nick && (
                                <span className="text-[0.72rem] text-brand font-mono tracking-wide">{leftSide.nick}</span>
                            )}
                        </div>
                        <div className={isReported && !isContested ? "font-['Bebas_Neue',sans-serif] text-[2.1rem] text-white [text-shadow:0_0_14px_rgba(199,149,255,0.45)] max-[900px]:text-[1.4rem]" : "font-['Bebas_Neue',sans-serif] text-[1.8rem] text-brand [text-shadow:0_0_12px_rgba(199,149,255,0.4)] max-[900px]:text-[1.4rem]"}>
                            {isReported && !isContested
                                ? `${score.player1} - ${score.player2}`
                                : "VS"}
                        </div>
                        <div className={`flex flex-col items-center gap-[0.35rem] p-4 px-2 border rounded-lg ${rightSide.isMe ? "border-[rgba(142,57,237,0.4)] bg-[rgba(142,57,237,0.08)]" : "border-[rgba(239,68,68,0.3)] bg-[rgba(239,68,68,0.05)]"}`}>
                            <span className="text-[0.72rem] font-bold tracking-[0.1em] text-text-soft">{rightLabel}</span>
                            <span className="text-[1.1rem] font-bold text-white text-center break-words">{rightName}</span>
                            {rightNick && (
                                <span className="text-[0.72rem] text-brand font-mono tracking-wide">{rightNick}</span>
                            )}
                        </div>
                    </div>
                    ...
                    {isContested ? (
                        <div className="mt-[-0.2rem] mb-[0.8rem] flex flex-col items-center gap-3">
                            <div className="flex items-center gap-2 px-4 py-3 rounded-lg border border-[rgba(251,191,36,0.4)] bg-[rgba(251,191,36,0.08)]">
                        ...
                        <div className="mt-[-0.2rem] mb-[0.8rem] flex flex-col items-center gap-3">
                            ...
                            {confirmadoPor.length > 0 && (
                                <div className="flex items-center gap-2 text-[0.78rem] text-text-soft">
                            ...
                            {canContest && showContestForm && (
                                <div className="w-full max-w-[420px] grid gap-2 mt-1">
                                    ...
                                    <textarea
                                        ...
                                        className="w-full box-border rounded-lg border border-[rgba(251,191,36,0.35)] bg-[rgba(255,255,255,0.04)] px-3 py-2 text-text-main text-[0.88rem] outline-none resize-y min-h-[72px] focus:border-[rgba(251,191,36,0.7)]"
                                    />
                                    <div className="flex gap-2 flex-wrap">
                                        <button
                                            type="button"
                                            className="inline-flex min-h-10 items-center justify-center px-4 py-2 border border-[rgba(251,191,36,0.45)] rounded-lg text-[0.88rem] font-semibold text-[#fde68a] bg-[rgba(251,191,36,0.12)] disabled:opacity-50"
                                            disabled={actionLoading}
                                            onClick={async () => {
                                        ...
                                        <button
                                            type="button"
                                            className="inline-flex min-h-10 items-center justify-center px-4 py-2 border border-[rgba(217,180,255,0.25)] rounded-lg text-[0.88rem] font-semibold text-text-soft bg-transparent"
                                            disabled={actionLoading}
                                            onClick={() => {
                                                setShowContestForm(false);
                                                setContestObservacao("");
                        ...
                        <div className="text-center">
                            <h3 className="m-0 mb-4 text-[0.95rem] font-semibold text-text-soft">Registrar Resultado</h3>
                            <div className="flex items-center justify-center gap-4 mb-4 max-[900px]:flex-col max-[900px]:gap-3">
                                <div className="flex flex-col items-center gap-2">
                                    <label className="text-[0.78rem] font-semibold text-text-soft max-w-[100px] max-md:max-w-[min(100%,12rem)] overflow-hidden text-ellipsis whitespace-nowrap">{leftSide.name}</label>
                                    <div className="flex items-center gap-[0.25rem]">
                                        <button
                                            type="button"
                                            className="w-9 h-9 border border-line rounded-lg bg-[rgba(255,255,255,0.05)] text-text-main text-[1.2rem] cursor-pointer flex items-center justify-center transition-all duration-[180ms] hover:bg-[rgba(167,79,255,0.2)] hover:border-line-strong"
                                ...
                                <span className="font-['Bebas_Neue',sans-serif] text-[1.4rem] text-text-soft mt-5 max-[900px]:mt-0">×</span>

                                <div className="flex flex-col items-center gap-2">
                                    <label className="text-[0.78rem] font-semibold text-text-soft max-w-[100px] max-md:max-w-[min(100%,12rem)] overflow-hidden text-ellipsis whitespace-nowrap">{rightName}</label>
                                    <div className="flex items-center gap-[0.25rem]">
                                        <button
                                            type="button"
                                            className="w-9 h-9 border border-line rounded-lg bg-[rgba(255,255,255,0.05)] text-text-main text-[1.2rem] cursor-pointer flex items-center justify-center transition-all duration-[180ms] hover:bg-[rgba(167,79,255,0.2)] hover:border-line-strong"
                            ...
                            {winsPlayer1 === 0 && winsPlayer2 === 0 && (
                                <p className="mt-2 text-[0.82rem] text-text-soft text-center opacity-75">
